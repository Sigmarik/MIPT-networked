...
static void draw_ship(float shipLen, float shipWidth, float x, float y,
                      const Vector2 &fwd, const Vector2 &left, Color col) {
...
static void draw_entity(const Entity &e) {
  ...
  draw_ship(shipLen + 0.4f, shipWidth + 0.4f, e.x, e.y, fwd, left,
            ColorFromHSV(int(hsv.x + 120.f) % 360, 1.f, 1.f));
  draw_ship(shipLen, shipWidth, e.x, e.y, fwd, left,
            ColorFromHSV(hsv.x, hsv.y, hsv.z)); // GetColor(e.color));
}
...
static void draw_world(const Camera2D &camera, const BandwidthAccumulator &bw) {
  ...
  DrawRectangleLines(-worldSize, -worldSize, 2.f * worldSize, 2.f * worldSize,
                     WHITE);

  constexpr size_t numGrid = 10;
  for (size_t y = 1; y < numGrid; ++y)
    DrawLine(-worldSize, -worldSize + 2.f * worldSize * (float(y) / numGrid),
             worldSize, -worldSize + 2.f * worldSize * (float(y) / numGrid),
             GetColor(0xffffffff));

  for (size_t x = 1; x < numGrid; ++x)
    DrawLine(-worldSize + 2.f * worldSize * (float(x) / numGrid), -worldSize,
             -worldSize + 2.f * worldSize * (float(x) / numGrid), worldSize,
             GetColor(0xffffffff));
  ...
  DrawText(TextFormat("Bandwidth: in %0.2f kbit/s",
                      get_delta_data(bw.inData) / 1024.f),
           8, 8, 12, WHITE);
  DrawText(TextFormat("Bandwidth: out %0.2f kbit/s",
                      get_delta_data(bw.outData) / 1024.f),
           8, 20, 12, WHITE);
  EndDrawing();
}
