...
static void draw_ship(float shipLen, float shipWidth, float x, float y, const Vector2 &fwd, const Vector2 &left,
                      Color col)
{
...
static void draw_world(const Camera2D &camera, const BandwidthAccumulator &bw)
{
  ...
  for (size_t y = 1; y < numGrid; ++y)
    DrawLine(-worldSize, -worldSize + 2.f * worldSize * (float(y) / numGrid), worldSize,
             -worldSize + 2.f * worldSize * (float(y) / numGrid), GetColor(0xffffffff));

  for (size_t x = 1; x < numGrid; ++x)
    DrawLine(-worldSize + 2.f * worldSize * (float(x) / numGrid), -worldSize,
             -worldSize + 2.f * worldSize * (float(x) / numGrid), worldSize, GetColor(0xffffffff));
